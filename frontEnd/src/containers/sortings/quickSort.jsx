...
const QuickSortVisualizer = () => {
    ...
    const quickSort = (arr, left = 0, right = arr.length - 1) => {
        const moves = [];
        if (left < right) {
            const partitionIndex = partition(arr, left, right, moves);
    ...
    const partition = (arr, left, right, moves) => {
        const pivotValue = arr[right];
        let partitionIndex = left;

        for (let i = left; i < right; i++) {
            if (arr[i] < pivotValue) {
                moves.push({ indices: [i, partitionIndex], type: 'swap' });
                [arr[i], arr[partitionIndex]] = [arr[partitionIndex], arr[i]];
                partitionIndex++;
            }
        }

        moves.push({ indices: [partitionIndex, right], type: 'swap' });
    ...
    const play = () => {
        ...
        const moves = quickSort(arrayCopy);
        animate(moves);
    };
    ...
    return (
        <div style={{ background: 'linear-gradient(135deg, #1a1a1a, #3f3f3f)', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <header style={styles.header}>
                <h1 style={styles.algorithmName}>Quick Sort Visualizer</h1>
                ...
                <div className="code-snippet" style={styles.codeSnippet}>
                    ...
                    <div style={styles.codeBlockWrapper}>
                        <SyntaxHighlighter 
                            ...
                            wrapLines={true}
                        >
                            {codeSnippets.quickSort[selectedLanguage]}
                        </SyntaxHighlighter>
                    </div>
                </div>
            </div>
